...
import { X, Send, Check, Sparkles, Loader2, ExternalLink } from 'lucide-react'
import { categoryStyles, statusStyles, fallbackBadge } from '../../lib/badges.js'

// Panel lateral de decisión sobre un correo: resumen IA, borrador editable,
// y acciones (responder / marcar resuelto).
// Se re-monta al cambiar de correo (key={email.id} en el padre).
export default function EmailDetail({ email, onClose, onReply, onResolve }) {
  const [draft, setDraft] = useState(email.borrador || '')
  const [busy, setBusy] = useState(null) // 'reply' | 'resolve'
  ...
  const run = async (kind, fn) => {
    ...
    try {
      await fn()
      setMsg(kind === 'reply' ? 'Respuesta enviada.' : 'Correo marcado como resuelto.')
    } catch (e) {
    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-black/50" onClick={onClose}>
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-xs text-slate-400">{email.hora} · {email.empresa || email.email}</p>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ring-1 ring-inset ${categoryStyles[email.categoria] ?? fallbackBadge}`}>
            {email.categoria}
          </span>
          <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${statusStyles[email.estado] ?? statusStyles.Pendiente}`}>
            {email.estado}
          </span>
          {email.webLink && (
            <a
        ...
        <label className="mt-5 block text-xs font-semibold text-slate-400">
          Borrador de respuesta (IA) — revísalo antes de enviar
        </label>
        <textarea
          rows={8}
          value={draft}
